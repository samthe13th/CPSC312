module Battleship where
  ...
  gameOver cGrid pGrid fb1 fb2 p c egp egc =
    do
      putStrLn ((drawBoard cGrid "computer") ++ divider ++ (drawBoard pGrid "human"))
      putStrLn fb1
      putStrLn fb2
      putStrLn ("\n" ++ egp ++ egc ++ "\n\nType \"start\" to begin another game.")
      restart <- getLine
      if (restart == "start") then (start) else (gameOver cGrid pGrid fb1 fb2 p c egp egc)
  ...
  continue cGrid pGrid fb1 fb2 p c line strategy =
    do
      ...
      let fb = line ++ "..." ++ (hitOrMiss cGrid (a,b)) ++ (isSunk (getCoordinates line) cGrid newGrid "human" c)
  ...
  computerPlay cGrid pGrid fb p c strategy =
    do
      ...
      let fb1 = "Firing on " ++ fb
      let newGrid = (attack pGrid (r,i) 0)
      let row = show (r + 1)
      let col = convertNum i
      let fb2 = c ++ "'s move: " ++ col ++ row ++ "..." ++ (hitOrMiss pGrid (r,i)) ++ (isSunk (r,i) pGrid newGrid "computer" c)
      newStrategy <- getStrategy strategy pGrid (r,i) newGrid
      let endGameP = (endGame cGrid "human" p)
      let endGameC = (endGame newGrid "computer" c)
      if (endGameP == "" && endGameC == "") then (personPlay cGrid newGrid fb1 fb2 p c newStrategy) else (gameOver cGrid newGrid fb1 fb2 p c endGameP endGameC)

  getStrategy strategy pGrid (r,i) ng
    | ((ng !! r) !! i) == '◼' && (isSunk (r,i) pGrid ng "computer" "any") == "" = return (show (r,i))
  ...
  hit (r,i) g = if ((legal (r,i)) && ((g !! r) !! i) == '◼') then True else False
...
--    | elem e "0123456789" = e
    | elem e "0123456789" = (if x == "computer" then '~' else '◻')
    | otherwise = e
